...
def init(request, template_name='MinersTracking/create_router.html'):
    ...
    if request.user.is_superuser:
        ...
        if profile.mine_id is not None:
            ...
            form = TrackingRouterForm()
            book = TrackingRouter.objects.filter(mine_id=-1)

    if request.method == "POST":
        form = TrackingRouterForm(request.POST or None)
        print(form.errors)
        try:
            if form.is_valid():
                form.save()
...
@csrf_protect
@login_required
def update_router(request, pk, template_name='MinersTracking/create_router.html'):
    current_user = request.user
    ...
    if request.user.is_superuser:
        ...
        form.fields['mine_id'].widget.attrs['disabled'] = True
    ...
    if request.is_ajax():
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                response_data['book'] = serializers.serialize('json',
                                                              TrackingRouter.objects.filter(pk=tracking_router_inst.pk),
                                                              fields=(
                                                              'mine_id', 'router_id', 'address', 'location', 'is_block',
                                                              'x_axis', 'y_axis', 'is_blocked', 'ip_add'))
...
def ajax_tracking_data(request):  ### for tabular format live data last 10 records
    ...
    if request.is_ajax():
        ...
        try:
            tracking_details = MinerTracking.objects.filter(mine_id=mine_id,
                                                            created_date__range=[date_from, date_to]).order_by('-id')[
                               :10]
...
def fetch_miner_ajax(request):
    ...
    if request.is_ajax():
        ...
        for r in router_details:
            ...
            router_model_data = {'title': str(r.router_id),
                                 'content': "<b>Address: </b>" + str(r.address) + "<br><b>Location: </b>" + str(
                                     r.location)}
...
def get_single_miner_path_data(request):
    ...
    if request.is_ajax():
        ...
        miner_table = MinerTracking.objects.filter(transmitter_id=miner_id, mine_id=mine_id,
                                                   created_date__range=[date_from, date_to]).order_by('id')
...
def get_all_miners_data(request):
    ...
    if request.is_ajax():
        ...
        for m in miner_details:
            ...
            # print(miner_id)
            try:
                tracking_table = MinerTracking.objects.filter(transmitter_id=miner_id, mine_id=mine_id).order_by(
                    '-id').first()
