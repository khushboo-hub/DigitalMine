...
def live_temp_data(request):
    ...
    graphical=[]
    if request.POST:
        if 'tabular' in request.POST:
            obj=temp_monitoring_automatic.objects.all()
        if 'graphical' in request.POST:
            graphical = temp_monitoring_automatic.objects.all()
    return render(request,'main/live_temp_data.html',{'tabular':obj,'graphical':graphical})
