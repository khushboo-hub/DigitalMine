            ...
            if (sensor_data):  # Alteast one data should be save in database
                ...
                if ((float(strata_data.level_1_warning_unit) < float(current_sensor_value)) and (
                    ...
                    strata_data_current.append(
                        {"module_type": "strata","is_strata_display_warning": is_strata_display_warning,"is_strata_audio":is_strata_audio,"is_strata_light":is_strata_light,"datetime": str(current_date_time.strftime("%d-%m-%Y %H:%M:%S")),"location": str(strata_data.location_id), "sensor_name": strata_data.sensor_name, "warning_level": "Low",
