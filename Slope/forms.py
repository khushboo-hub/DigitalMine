...
from .models import Slope_location, Slope_sensor, Slope_sensor_flag
from datetime import datetime


class Slope_location_Form(forms.ModelForm):
    ...
    json_url = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Online MQTT/Json URL'}))
    ...
    class Meta():
        model = Slope_location
        fields = ['mine_name','location_name' ,'tag_no','x_axis','y_axis','json_url','description','created_date']

class Slope_sensor_Form(forms.ModelForm):
    SENSOR_NAME = (
        ('tiltmeter', 'Tiltmeter'),
        ('inclinometer', 'Inclinometer'),
        ('rainguage', 'Rainguage'),
        ('pizometer', 'Pizometer'),
        ('crackometer', 'Crackometer'),
    )
    sensor_name = forms.CharField(widget=forms.Select(choices=SENSOR_NAME, attrs={'class': 'form-control', }), )
    ...
    level_1_msg = forms.CharField(required=False, initial='Slope Condition:First Stage Warning Message , Please be Careful.', widget=forms.TextInput(attrs={'class': 'form-control', }))
    level_2_msg = forms.CharField(required=False, initial='Slope Condition:Second Stage Warning Message , Please be alert situation is not good.',widget=forms.TextInput(attrs={'class': 'form-control', }))
    level_3_msg = forms.CharField(required=False, initial='Slope Condition:Third Stage Warning Message , Please  leave the underground',widget=forms.TextInput(attrs={'class': 'form-control', }))
...
class Slope_sensor_flag_Form(forms.ModelForm):
